#!/usr/bin/env bash
# JumpCloud Command: IS > Prod > Mac + Linux > Deploy SonarQube CLI
# Runs daily as SYSTEM. Installs the CLI to /usr/local/bin on first run only.
...
if [[ -f "$MDM_BINARY" ]]; then
    echo "SonarQube CLI already present at $MDM_BINARY — skipping (upgrades via MDM update script)."
    exit 0
fi

echo "SonarQube CLI not found — installing to $MDM_BINARY..."
