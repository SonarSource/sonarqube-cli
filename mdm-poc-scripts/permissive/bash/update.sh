#!/usr/bin/env bash
# JumpCloud Command: IS > Prod > Mac + Linux > Update SonarQube CLI
# Downloads and installs the latest CLI version directly, replacing the existing MDM binary.
# FORCE            -- optional JumpCloud variable; if set, replaces even when already at latest.
...
latest_version="$(curl -fsSL --proto "$HTTPS_ONLY" --proto-redir "$HTTPS_ONLY" "${ARTIFACT_BASE_URL}/stable.version" | tr -d '[:space:]')"
echo "Latest version: $latest_version"

latest_semver="$(echo "$latest_version" | grep -oE '^[0-9]+\.[0-9]+\.[0-9]+')"

if [[ -f "$MDM_BINARY" ]]; then
    installed_semver="$("$MDM_BINARY" --version 2>/dev/null | grep -oE '^[0-9]+\.[0-9]+\.[0-9]+' || true)"
    if [[ -z "${FORCE:-}" && "$installed_semver" == "$latest_semver" ]]; then
        echo "Already at version $latest_version, nothing to do."
        exit 0
    fi
    echo "Updating $installed_semver -> $latest_semver..."
else
    echo "MDM binary not found -- installing..."
    mkdir -p "$(dirname "$MDM_BINARY")"
fi

download_binary "$latest_version" "$MDM_BINARY"
verify_sha256 "$MDM_BINARY" "$latest_version"
