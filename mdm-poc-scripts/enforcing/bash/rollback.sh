#!/usr/bin/env bash
# JumpCloud Command: IS > Prod > Mac + Linux > Rollback SonarQube CLI (enforcing)
# Ensures the MDM binary is present, then rolls back to ROLLBACK_VERSION.
# ROLLBACK_VERSION — required JumpCloud variable (e.g. 1.1.0.3122).
