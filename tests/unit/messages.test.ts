...
describe('messages: mock mode', () => {
  ...
  it('text records call', () => {
    text('plain text');
    expect(getMockUiCalls().some(c => c.method === 'text' && c.args[0] === 'plain text')).toBe(true);
  });

  it('print records call', () => {
    print('raw output');
    expect(getMockUiCalls().some(c => c.method === 'print' && c.args[0] === 'raw output')).toBe(true);
  });
...
describe('messages: real output (non-mock)', () => {
  it('info writes to stdout with ℹ prefix', () => {
    const output: string[] = [];
    const spy = spyOn(process.stdout, 'write').mockImplementation((s) => { output.push(String(s)); return true; });
    try {
      info('test message');
  ...
  it('warn writes to stderr', () => {
    const output: string[] = [];
    const spy = spyOn(process.stderr, 'write').mockImplementation((s) => { output.push(String(s)); return true; });
    try {
      warn('be careful');
