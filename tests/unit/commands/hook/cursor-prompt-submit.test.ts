...
// Unit tests for cursorPromptSubmit — only paths that cannot be exercised via integration tests:
...
describe('cursorPromptSubmit (unit — impractical-via-e2e paths)', () => {
  ...
  it('writes a block decision with generic user_message when secrets are found', async () => {
    ...
    await cursorPromptSubmit(makeCtx().ctx);
    ...
    expect(payload.user_message).toBe('Sonar detected secrets in prompt');
  });
