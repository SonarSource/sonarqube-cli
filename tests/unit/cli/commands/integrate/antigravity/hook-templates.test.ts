...
} from '../../../../../../src/cli/commands/integrate/antigravity/hook-templates';

describe('Antigravity secret hook templates', () => {
  it('Unix template delegates to sonar hook antigravity-pre-tool-use', () => {
    const template = getSecretPreToolTemplateUnix();

    expect(template.startsWith('#!/bin/bash')).toBe(true);
    expect(template.includes('sonar hook antigravity-pre-tool-use')).toBe(true);
    expect(template.includes(UNIX_SONAR_COMMAND_GUARD)).toBe(true);
    expect(template.includes('sonar analyze')).toBe(false);
  });

  it('Windows template delegates to sonar hook antigravity-pre-tool-use', () => {
