...
`;
    ...
    it('strips a leading --- document marker so the job is not placed in a second, ignored YAML document', async () => {
      const { gitlabServer } = await startServers(harness, {
        gitlabProjects: [
          {
            id: 103,
            name: 'existing-ci-repo',
            path_with_namespace: `${GROUP}/existing-ci-repo`,
            default_branch: 'main',
            rootFiles: [{ name: '.gitlab-ci.yml', content: 'stages:\n  - build\n  - test\n' }],
          },
        ],
        bindings: [
          {
            projectKey: 'mycompany_existing-ci-repo',
            repository: '103',
            dopSettingId: DOP_SETTING_ID,
          },
        ],
      });
      harness.cwd.writeFile('template.yml', `---\n${JOB_TEMPLATE}`);
      ...
      const ciYml = gitlabServer.committedFiles.find((f) => f.path === '.gitlab-ci.yml');
      expect(ciYml).toBeDefined();
      expect(ciYml!.content).not.toContain('---');
      const parsed = yaml.load(ciYml!.content) as { stages: string[]; 'security-scan': unknown };
      expect(parsed.stages).toEqual(['build', 'test']);
      expect(parsed['security-scan']).toBeDefined();
    });

    it('rejects a --job-template whose document marker is preceded by a comment header', async () => {
      await startServers(harness);
      harness.cwd.writeFile('template.yml', `# Org template\n---\n${JOB_TEMPLATE}`);
      ...
      expect(result.exitCode).toBe(2);
      expect(result.stderr).toContain('must not contain YAML document markers');
    });

    it('rejects a --job-template whose document marker has a trailing comment', async () => {
      await startServers(harness);
      harness.cwd.writeFile('template.yml', `---   # start\n${JOB_TEMPLATE}`);
