...
describe('integrate cursor', () => {
  ...
  describe('install (global by default)', () => {
    it(
      'writes ~/.cursor/mcp.json with a sonarqube MCP server entry, not to the project directory',
      async () => {
    ...
    it(
      ...
      async () => {
        ...
        expect(
          (harness.userHome.file(...MCP_JSON_DIRS).asJson() as CursorMcpFile).mcpServers?.sonarqube,
    ...
    it(
      'records mcp-server feature in state with global scope',
      async () => {
        const server = await harness
          .newFakeServer()
          .withAuthToken('test-token')
    ...
    it(
      'omits the project key from MCP server args even when discovered (global install has no fs mount)',
      async () => {
        const server = await harness
          .newFakeServer()
          .withAuthToken('test-token')
        ...
        expect(mcp.mcpServers?.sonarqube?.args).not.toContain('my-project');
      },
      { timeout: 30000 },
    );
    ...
    it(
      ...
      async () => {
        await harness.run('integrate cursor --non-interactive');
        const firstBody = harness.userHome.file(...MCP_JSON_DIRS).asText();

        const result = await harness.run('integrate cursor --non-interactive');

        expect(result.exitCode).toBe(0);
        expect(harness.userHome.file(...MCP_JSON_DIRS).asText()).toBe(firstBody);
    ...
    it(
      ...
      async () => {
        harness.userHome.writeFile('.cursor/mcp.json', '{ invalid json');
  ...
  describe('secrets scanning hooks', () => {
    ...
    it(
      ...
      async () => {
        ...
        const preReadScript = harness.userHome.file(
          ...HOOK_BUILD_SCRIPT_DIRS,
          hookScriptName('before-read-file-secrets'),
        );
        const preToolScript = harness.userHome.file(
          ...HOOK_BUILD_SCRIPT_DIRS,
    ...
    it(
      'uses an absolute command path anchored under the home directory',
      async () => {
        const result = await harness.run('integrate cursor --non-interactive');
    ...
    it(
      ...
      async () => {
        ...
        const feature = findInstalledFeature(harness, 'cursor', 'sonar-secrets-hooks', 'global');
  ...
  describe('Vortex entitlement and SQAA instructions', () => {
    ...
    it(
      'writes an always-applied ~/.cursor/rules/sonar-agentic-analysis.mdc when entitled, with a discovered project key',
      async () => {
        ...
        expect(result.exitCode).toBe(0);
        expect(harness.userHome.exists(...SQAA_RULE_DIRS)).toBe(true);

        const body = harness.userHome.file(...SQAA_RULE_DIRS).asText();
        ...
        expect(findInstalledFeature(harness, 'cursor', 'vortex', 'global')).toBeDefined();
      },
      { timeout: 30000 },
    );
    ...
    it(
      ...
      async () => {
        ...
        const session = harness.runInteractive('integrate cursor', {
    ...
    it.each([
      ...
      async (isAgent, isInteractive, expectedShownPrompt) => {
        ...
        if (isInteractive) {
          ...
          result = await harness.run('integrate cursor --non-interactive', { extraEnv: runEnv });
        }

        expect(result.exitCode).toBe(0);
        if (expectedShownPrompt) {
          expectAgentPromptHint(result.stdout, 'sonar integrate cursor --non-interactive');
        } else {
    ...
    it(
      ...
      async () => {
        ...
        expect(harness.userHome.file(...SQAA_RULE_DIRS).exists()).toBe(false);
    ...
    it(
      ...
      async () => {
        ...
        await harness.run('integrate cursor --non-interactive', { extraEnv });
        const firstBody = harness.userHome.file(...SQAA_RULE_DIRS).asText();

        const result = await harness.run('integrate cursor --non-interactive', { extraEnv });

        expect(result.exitCode).toBe(0);
        expect(harness.userHome.file(...SQAA_RULE_DIRS).asText()).toBe(firstBody);
      },
      { timeout: 30000 },
    );
    ...
    it(
      ...
      async () => {
        ...
        expect(result.exitCode).toBe(0);
        expect(harness.userHome.exists(...SQAA_RULE_DIRS)).toBe(false);
      },
      { timeout: 30000 },
    );
  });
