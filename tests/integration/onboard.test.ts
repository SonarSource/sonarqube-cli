...
`;
    ...
    expect(projectInfo.sonarPropsData!.organization).toBe('test-org');
    ...
    const state: CliState = {
      ...
      auth: {
        ...
        connections: [
          {
            id: 'id',
            serverUrl: projectInfo.sonarPropsData!.hostURL,
            type: 'cloud',
            orgKey: projectInfo.sonarPropsData!.organization,
            authenticatedAt: '0',
            keystoreKey: 'key',
          },
        ],
...
it('integration: onboard with existing .sonarlint config', async () => {
  ...
  try {
    ...
    const state: CliState = {
      ...
      auth: {
        ...
        connections: [
          {
            id: 'id',
            serverUrl: projectInfo.sonarLintData!.serverURL,
            type: 'on-premise',
            authenticatedAt: '0',
            keystoreKey: 'key',
          },
    ...
    expect(loaded.auth.connections[0].serverUrl).toBe('https://sonarqube.example.com');
...
it(
  'integration: auth login process exits after token delivered to loopback server',
  async () => {
    ...
    const proc = Bun.spawn(
      ['bun', 'run', 'src/index.ts', 'auth', 'login', '--server', fakeServer],
      {
        ...
        env: {
          ...process.env,
          CI: 'true',
          SONAR_CLI_DISABLE_KEYCHAIN: 'true',
          SONAR_CLI_DIR: isolatedCliDir,
        },
      },
    );
