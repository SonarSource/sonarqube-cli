...
// Shared CAG state seeding: a stale declarative Vortex/CAG feature so
// `runPostUpdateActions()` has something to reconcile.
...
import { expectVortexHookInstalled, sessionStartScript } from './cag-helpers';
import { Dir } from './dir';
...
export interface SeedStateOptions {
  ...
  /** Copy the CAG stub into `<cliHome>/bin` so post-update does not download. */
  installCagStub?: boolean;
}
...
export function seedState(harness: TestHarness, options: SeedStateOptions = {}): void {
  ...
  if (options.installCagStub) {
    const binaryPath = join(harness.cliHome.path, 'bin', buildLocalCagBinaryName(detectPlatform()));
