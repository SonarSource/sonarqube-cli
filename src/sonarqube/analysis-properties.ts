...
// Parser for the subset of `/api/settings/values` results consumed by SCA
...
export interface ProjectAnalysisProperties {
  scaProperties: Record<string, string>;
  exclusions: string[];
  includeGitIgnoredPaths: boolean;
}

export function parseAnalysisProperties(settings: SettingsValue[]): ProjectAnalysisProperties {
  ...
  for (const setting of settings) {
    if (EXCLUSION_KEYS.has(setting.key)) {
      exclusions.push(...parseExclusions(setting));
...
function parseExclusions(setting: SettingsValue): string[] {
  if (setting.values) {
