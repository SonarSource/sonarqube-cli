...
export async function runPreCommitInstall(root: string, hook: GitHookType): Promise<void> {
  await spawnProcess('pre-commit', ['uninstall'], { cwd: root });
  await spawnProcess('pre-commit', ['clean'], { cwd: root });
  await spawnProcess('pre-commit', ['install'], { cwd: root });
  if (hook === 'pre-push') {
    await spawnProcess('pre-commit', ['install', '--hook-type', 'pre-push'], { cwd: root });
  }
}
