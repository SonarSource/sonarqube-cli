...
import { HOOK_MARKER, getHookScript } from './git-scripts';
...
function detectHookInstallation(root: string): HookInstallation {
  return {
    ...
    gitPreCommit: hasMarker(join(root, '.git', 'hooks', 'pre-commit')),
    gitPrePush: hasMarker(join(root, '.git', 'hooks', 'pre-push')),
  };
}
...
function showInstallationStatus(root: string): void {
  const installed = detectHookInstallation(root);
...
async function installViaPreCommitFramework(root: string, hook: GitHookType): Promise<void> {
  ...
  try {
    ...
    throw new CommandFailedError(
      `Updated ${PRE_COMMIT_CONFIG_FILE} but pre-commit commands failed. Install the pre-commit framework (e.g. pip install pre-commit) and run: pre-commit uninstall && pre-commit clean && pre-commit install${hook === 'pre-push' ? ' && pre-commit install --hook-type pre-push' : ''}`,
    );
  }
...
async function integrateGitGlobal(options: IntegrateGitOptions): Promise<void> {
  warn('WARNING: Global hook installation');
  text('  This will set git config --global core.hooksPath to:');
  text(`  ${GLOBAL_HOOKS_DIR}`);
  text('  ALL repositories on this machine will use this hooks directory.');
  text('  Any existing .git/hooks/ scripts in individual repos will be bypassed.');
  blank();
  ...
  await fs.writeFile(hookPath, getHookScript(hook), { mode: 0o755 });
  await spawnProcess('git', ['config', '--global', 'core.hooksPath', GLOBAL_HOOKS_DIR]);
...
export async function integrateGit(options: IntegrateGitOptions): Promise<void> {
  ...
  if (!projectInfo.isGitRepo) {
    throw new CommandFailedError(
      'No git repository found. Please run this command from inside a git repository.',
    );
  }
  ...
  showPostInstallInfo(hook);
  showInstallationStatus(projectInfo.root);
  note('Run: sonar integrate git test', 'Verify the hook works');
}
...
export async function integrateGitTest(): Promise<void> {
  ...
  if (!projectInfo.isGitRepo) {
    throw new CommandFailedError(
      'No git repository found. Run this command from inside a git repository.',
    );
  }

  const installed = detectHookInstallation(projectInfo.root);
  if (!installed.preCommitConfig && !installed.huskyPreCommit && !installed.gitPreCommit) {
    throw new CommandFailedError('Pre-commit hook not found. Install with: sonar integrate git');
  }
  ...
  try {
    ...
    throw new CommandFailedError(
      `Could not create test file ${TEST_FILE_NAME}: ${(err as Error).message}`,
    );
  }
  ...
  try {
    ...
    if (!combined.includes('Found') && !combined.toLowerCase().includes('secret')) {
      await cleanup();
      throw new CommandFailedError(
        'Hook test failed: the commit was blocked but the output did not mention a secret. It may have been blocked for a different reason.',
      );
    }

    success('Hook test passed: the commit was blocked as expected.');
    text('  The hook detected the fake secret in ' + TEST_FILE_NAME + '.');
  } finally {
    await cleanup();
  }
}
