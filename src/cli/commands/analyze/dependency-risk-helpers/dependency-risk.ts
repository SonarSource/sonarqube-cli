...
export interface DependencyRiskVersionOption {
  version: string;
...
export interface DependencyRisk {
  ...
  cvssScore?: string;
  cweIds?: string[];
  versionOptions?: DependencyRiskVersionOption[];
}
...
function toDependencyRisk(
  ...
  if (issue.type === 'VULNERABILITY') {
    ...
    risk.versionOptions = issue.versionOptions?.map(toVersionOption);
...
function toVersionOption(option: VersionOption): DependencyRiskVersionOption {
  return {
    version: option.version,
    vulnerabilityIds: option.vulnerabilityIds,
    prerelease: option.prerelease,
    fixLevel: option.fixLevel,
