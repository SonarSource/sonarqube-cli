...
export interface ClaudeHookEventContainerConfig<TOptions = Record<string, unknown>> {
  ...
  event: 'PostToolUse';
  configDir: string;
  /** Marker embedded in the generated command — identifies entries this container owns. */
  marker: string;
  scriptPath: string;
...
/** True when at least one subfeature could actually install — else the container would leave a phantom entry. */
