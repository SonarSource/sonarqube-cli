...
import { bold, dim, lavender } from '@/core/ui/colors.ts';
...
async function resolveStatsEntitlement(ctx: CommandInvocationContext): Promise<StatsEntitlement> {
  const auth = await ctx.resolveAuthOrNull({ silent: true });
  if (!auth) {
  ...
  const [vortex, sca] = await Promise.all([
    ...
    client.getScaEnablement(auth.connectionType, auth.orgKey).orThrow(),
  ]);
...
export async function sonarStats(
  ...
  if (options.json) {
    const entitlement = summary.totalRuns > 0 ? await resolveStatsEntitlement(ctx) : null;
...
function printEmptyState(console: Console): void {
  ...
  console.blank();
  console.print(`  ${dim('→ Install them:')} ${lavender('sonar integrate')}`);
}
...
function printAgentBreakdownSection(summary: StatsSummary, console: Console): void {
  ...
  if (summary.agentHitRate) {
    ...
    console.print(`    ${hitPercent}% of agent runs surfaced an issue`);
  }
