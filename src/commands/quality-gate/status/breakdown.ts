...
export function hasFailingConditionInCategory(
  ...
  return conditions.some((condition) => isFailingConditionInCategory(condition, category));
}

/**
 * A failing condition whose metric falls into an implemented category, filtered to `category`
 * when given.
 */
function isFailingConditionInCategory(
  condition: QualityGateCondition,
  category: string | undefined,
): boolean {
  const conditionCategory = METRIC_CATEGORIES[condition.metricKey];
  return (
    condition.status !== 'OK' &&
    !!conditionCategory &&
    (!category || category === conditionCategory)
  );
}
...
export async function buildBreakdown(
  ...
  const results = await Promise.all(
    params.conditions
      .filter((condition) => isFailingConditionInCategory(condition, params.category))
