...
**Vortex** is the umbrella for SQAA and Context Augmentation. For **Claude Code** and **Copilot** the two are delivered as one `vortex` container feature (`integrate/_common/vortex.ts` `createVortexFeature`, wired in `integrate/claude/declaration.ts` and `integrate/copilot/declaration.ts`) whose subfeatures are `sonar-sqaa-hook` (Claude only, since Copilot has no SQAA hook), `sqaa-instructions`, and `context-augmentation` — the ids those capabilities had as standalone features, so `replacedIds` migrates pre-unification installs into the container on post-update (`defaultInstallSubfeatureIds` decides what a migrated record gets). One prompt (`Install Vortex? (analyzes edits and enriches prompts)`) installs all of them; the CAG subfeature comes from the shared `createContextAugmentationSubfeature` factory, which other agents still wrap as a standalone feature via `createContextAugmentationFeature`. `resolveVortexSetup` (`integrate/_common/vortex-setup.ts`) runs the single entitlement check and resolves the SCA flag. Runtime readers accept the container plus the legacy standalone record while other agents are unmigrated: `isProjectVortexFeature` (`integrate/_common/vortex.ts`) is combined with the old feature id in `resolveSqaaProjectKey` (`analyze/sqaa-auth.ts`) and the `sonar context` passthrough. The other agents are not migrated yet and keep their standalone SQAA/CAG features.
...
Before installing, `sonar integrate claude|copilot|codex|antigravity` pre-flights a unified **Vortex** entitlement check gating both SQAA and CAG: `SonarQubeClient.hasVortexEntitlement(orgKey)` resolves the org UUID once (SonarQube Cloud only), then checks the SQAA (`/a3s-analysis/org-entitlement/{uuid}`) and CAG (`/cag/cag-entitlement/{uuid}`) endpoints in parallel via `checkSqaaEntitlement`/`checkCagEntitlement`. Each returns a shared `VortexEntitlementStatus` (`enabled`, `over_consumption`, `not_entitled`, `check_failed`), combined with precedence `check_failed > not_entitled > over_consumption > enabled`. `resolveVortexSetup` (Claude and Copilot) and `resolveSqaaSetup`/`resolveContextAugmentationSetup` (other agents) delegate to it and install on `enabled` or `over_consumption` (skipping on `not_entitled`/`check_failed`), so consumption never blocks install — only runtime tool calls (enforced by the CAG daemon); `sonar context` is not gated.
