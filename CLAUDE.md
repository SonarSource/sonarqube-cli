...
`sonar onboard` is the one-command machine-wide setup flow: after `sonar auth login`, it detects installed agents and runs each agent's integrate handler with `{ global: true, onboardMode: true, nonInteractive: true }`. Sonar-linked project keys are resolved at runtime (`resolveRuntimeProjectKey()` in `src/lib/runtime-project-context.ts`) — hooks, analyze, MCP, and `sonar context` discover the project from the current working directory; no per-repo integrate is required. Global onboard installs the CAG skill file but skips `tool integrate` (daemon bootstrap) until the first `sonar context` use inside a Sonar-linked repo. Cursor SQAA always-applied rules remain project-scoped; global onboard still installs secrets hooks and MCP for Cursor. Git pre-commit hooks are not part of onboard. Flags: `--yes`, `--agents <csv>`, `--minimal` (skip SQAA), `--skip-context`. Implementation in `src/cli/commands/onboard/`.
