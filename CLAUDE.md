...
Release builds publish standalone executables for `linux-x86-64`, `linux-arm64`, `macos-arm64`, and `windows-x86-64`. binaries.sonarsource.com artifacts use the `.bin` suffix on Linux/macOS and `.exe` on Windows (e.g. `sonarqube-cli-{version}-linux-x86-64.bin`); versions published before that convention remain `.exe` on the CDN. Dependency binaries (sonar-secrets, sca-scanner-cli) still use `.exe` in download URLs. The stable installers (`user-scripts/install.sh`, `user-scripts/install.ps1`) resolve the real release version from `Distribution/sonarqube-cli/stable.version`; the shell installers select the Linux artifact using `uname -m` (`aarch64` / `arm64` → `linux-arm64`, `x86_64` / `amd64` → `linux-x86-64`) and try `.bin` then `.exe` when downloading. They also keep a literal compatibility version marker for older `sonar self-update` clients, and `full-release.yml` updates that marker to the latest released version during the post-release bump PR. Both scripts accept an optional install-directory override: `--install-dir <path>` (install.sh) and `-InstallDir <path>` (install.ps1). When an explicit directory is provided, the version-skip optimisation is bypassed so the correctly-flavored binary is always installed; on Windows a non-default directory triggers Machine-scope PATH registration (requires elevation — the MDM/SYSTEM context always has it). Both scripts also accept `--artifact-base-url <url>` / `-ArtifactBaseUrl <url>` to override the binary download base URL — useful for air-gapped or internal-mirror deployments where binaries are hosted at a customer-controlled location rather than `binaries.sonarsource.com`. `sonar self-update` forwards this flag to the install script via `--artifact-base-url` (Unix) and `-ArtifactBaseUrl` (Windows).
