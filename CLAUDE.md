...
- Shared Sonar product data lives under `~/.sonar` (`SONAR_USER_HOME` override in tests); CLI-specific data stays under `~/.sonar/sonarqube-cli`, and the stable anonymous telemetry user ID is shared at `~/.sonar/user`.
