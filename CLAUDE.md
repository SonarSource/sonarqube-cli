...
- `night-owl.md` runs nightly at `00:00 UTC`, installs Atlassian CLI in a prep job, authenticates with Vault-provided Jira bot credentials, selects the first `CLI` Jira ticket labeled `for-agent` in status `Open`, `TODO`, or `To Do`, materializes the ticket plus parent/comment/link context, posts Slack when the queue is starving or prep fails, then lets the agent work only from that prepared Jira context and open draft PRs.
- `ci-failure-triage-agent.md` is temporarily repurposed to mirror `night-owl.md` so the Night Owl flow can be triggered from the existing workflow slot on `master` during rollout.
