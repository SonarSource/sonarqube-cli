...
`sonar import` (hidden while in development — Step 1 of MMF-5732) imports a repository from a connected DevOps platform into SonarQube. The provider is determined internally from the selected DevOps platform config, not from a user-facing argument. Accepts `--org <key>` (SonarQube organization key, prompted interactively when omitted) and `--non-interactive`. Implementation lives in `src/cli/commands/import/`; shared resolution helpers in `src/cli/commands/import/_common/`. The command will become visible (with `rootHelp({ category: 'data' })`) once DevOps platform config selection, repo selection, and the import API call are wired up.
