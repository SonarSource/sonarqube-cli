...
**Vortex** is the umbrella for SQAA and Context Augmentation. For **Claude Code**, **Copilot**, and **Cursor** the two are delivered as one `vortex` container feature (`integrate/_common/vortex.ts` `createVortexFeature`, wired in each integration's declaration). Every container has `sqaa-instructions` and `context-augmentation` subfeatures; Claude also has `sonar-sqaa-hook`. These are the ids the capabilities had as standalone features, so `replacedIds` migrates pre-unification installs into the container on post-update (`defaultInstallSubfeatureIds` decides what a migrated record gets). One prompt (`Install Vortex? (analyzes edits and enriches prompts)`) installs all of them; the CAG subfeature comes from the shared `createContextAugmentationSubfeature` factory, which unmigrated agents still wrap as a standalone feature via `createContextAugmentationFeature`. `resolveVortexSetup` (`integrate/_common/vortex.ts`) runs the single entitlement check and resolves the SCA flag. Runtime readers accept the container plus the legacy standalone record while Codex and Antigravity remain unmigrated: `isProjectVortexFeature` is combined with the old feature id in `resolveSqaaProjectKey` (`analyze/sqaa-auth.ts`) and the `sonar context` passthrough.
...
Before installing, `sonar integrate claude|copilot|cursor|codex|antigravity` pre-flights a unified **Vortex** entitlement check gating both SQAA and CAG: `SonarQubeClient.hasVortexEntitlement(orgKey)` resolves the org UUID once (SonarQube Cloud only), then checks the SQAA (`/a3s-analysis/org-entitlement/{uuid}`) and CAG (`/cag/cag-entitlement/{uuid}`) endpoints in parallel via `checkSqaaEntitlement`/`checkCagEntitlement`. Each returns a shared `VortexEntitlementStatus` (`enabled`, `over_consumption`, `not_entitled`, `check_failed`), combined with precedence `check_failed > not_entitled > over_consumption > enabled`. `resolveVortexSetup` (Claude, Copilot, and Cursor) and `resolveSqaaSetup`/`resolveContextAugmentationSetup` (other agents) delegate to it and install on `enabled` or `over_consumption` (skipping on `not_entitled`/`check_failed`), so consumption never blocks install — only runtime tool calls (enforced by the CAG daemon); `sonar context` is not gated.
