...
`sonar analyze dependency-risks` pre-scans discovered manifest files for secrets (via `sonar-secrets`) before the SCA scan and aborts if any are found. In the `analyze` path the secrets binary is a hard prerequisite (install failure aborts the run); the git pre-push hook path fails open (skips/warns). See `dependency-risk-helpers/manifest-secrets-guard.ts`.
