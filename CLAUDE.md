...
- E2E tests: `tests/e2e/` — real external dependencies that cannot be faked: OS keychain, install scripts with real network, real SonarQube server calls, and integration with external tools. Those tests are black-box tests and exercise the product from the outside. `tests/e2e/context/` is the offline real-binary suite for `sonar-context-augmentation`: it seeds CAG state in `state.json`, lets `runPostUpdateActions()` re-download CAG from `binaries.sonarsource.com`, and covers post-update refresh (`cag-offline.test.ts`), edge cases like missing project roots / global skills / multi-skill refresh / stale-binary cleanup (`cag-edge-cases.test.ts`), passthrough behaviors including unauthenticated errors and exit-code propagation (`cag-passthrough.test.ts`), the `copilot-cli` skill path (`cag-copilot.test.ts`), the `codex` skill path (`cag-codex.test.ts`), and `sonar integrate` pre-flight skip paths against a fake server — SonarQube Server connections and disabled entitlement (`cag-integrate.test.ts`; the happy path is deferred because CAG's daemon socket path exceeds the AF_UNIX limit under the harness `tmpdir` layout). Shared scaffolding lives in `_helpers.ts`. No SonarQube/Cloud access required — only the binaries CDN. `tests/e2e/sca/sca-staging.test.ts` (CLI-452) is a credential-gated EU+US SonarQube Cloud staging suite for `sonar analyze dependency-risks`: per region it provisions a project, scans the vulnerable npm fixture (`tests/e2e/sca/fixtures/vulnerable-npm-project/`) with the real `sca-scanner-cli`, asserts a risk is found (exit 51), and deletes the project. Region-aware staging + project helpers live in `tests/e2e/_common/staging.ts`. A region is skipped unless its token is set: `SONARCLOUD_IT_TOKEN` (EU) / `SONARCLOUD_IT_TOKEN_US` (US), from 1Password "SonarLint Core - SonarCloud ITs"; org is fixed `sonarlint-it`.
