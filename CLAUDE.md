...
- `.claude/rules/state-and-auth.md` — `state.json`, the keychain, auth resolution, project discovery (`discoverProject()`). Its callers are `src/core/state/`, `src/core/auth/`, four top-level `src/core/` modules and the `auth` / `link` commands.
- `.claude/rules/sonarqube-api.md` — network access (proxy/TLS, fetch wrappers) and the SonarQube API client layer. Clients live both in `src/core/server/` and next to the commands that own them.
