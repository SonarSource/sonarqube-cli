...
2. **Disk cache** — `{SONAR_USER_HOME}/sonarqube-cli/telemetry/identity-cache.json`, one entry per auth fingerprint (`src/core/host/identity-fetch.ts`). Avoids repeat API calls across CLI invocations, independently of the connection seed.
...
**Field completeness** (`isIdentityCompleteForConnection`, in `src/core/host/identity-fetch.ts`):
