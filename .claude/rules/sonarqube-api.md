...
**Every API client is constructed from a `SonarHttpClient`**, never from a `(serverUrl, token)` pair it turns into one itself. That transport comes from the invocation's `SonarConnection` (`src/core/server/connection.ts`) — `auth` plus the transport bound to its server — so there is a single way to obtain a client: `ctx.connection` on an authenticated handler, `await ctx.resolveConnection()` on the parent context, which returns `null` when the invocation has no usable credentials (hooks and `system status` rely on that instead of resolving auth and building a transport themselves). Helpers below a handler take the whole connection rather than a `(transport, auth)` pair, so the organization key cannot drift from the transport it belongs to. Code with no invocation context to read it from — framework/integrate helpers that take `auth` rather than a context, and telemetry/auth code that runs before a connection exists or outside any command — builds one inline and threads it down. A test that needs a fake transport for code reading it from a context overrides `createCliRuntime({ httpClientFactory })` instead of stubbing `SonarHttpClient.prototype`; the context-less code above has no seam to override, so its tests still stub the prototype.
